...
description: >
  How the Mapping Specialist maps a vendor catalogue product to a JPMC CDAO
  taxonomy node. Use this whenever the task is to classify, match, reconcile, or
  propose a taxonomy target for a vendor product (LSEG, S&P, Bloomberg, ICE,
  FactSet) — even when the request says "classify" or "find the right category"
  rather than "map". This is the specialist's core job; do not attempt a mapping
  without it.
allowed-tools: graphrag_retrieve neptune_node_by_iri neptune_existing_mapping neptune_conflicts rdf_serialise_mapping
---
