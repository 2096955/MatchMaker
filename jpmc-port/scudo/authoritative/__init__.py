...
def existing_mapping(vendor: str, vendor_product_ref: str) -> Optional[dict]:
    ...
    if _is_real_mode():
        ...
        return neptune_mcp.existing_mapping(
...
def taxonomy_snapshot() -> Optional[list[dict]]:
    """Return an authoritative complete topology only when implemented.

    The current real MCP facade has bounded single-node queries but no complete
    snapshot operation, so production fails closed instead of substituting the
    lexical sidecar or making an unbounded network query.
    """
    return None


def illustrative_taxonomy_snapshot() -> list[dict]:
    """Return the coherent, clearly illustrative local fixture."""
    from . import mock

    return mock.illustrative_taxonomy_snapshot()


__all__ = [
    "conflicts",
    "existing_mapping",
    "illustrative_taxonomy_snapshot",
    "node_by_iri",
    "taxonomy_snapshot",
]
