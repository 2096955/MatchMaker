---
type: Plan
title: Dense ARM SDK Adoption (v0.3)
description: Replan that ships the Opus-4.8 prompt dense arm for similarity scoring
  now and DROPS/PARKS the GraphRAG-SDK + Titan vector path behind the SCUDO_DENSE_BACKEND
  flag; supersedes dense-arm-swap v0.2.
tags:
- plan
- dense-arm
staleness: current
supersedes: /plans/dense-arm-swap.md
...
- **Fusion:** RRF (rank-only). `Candidate.similarity = raw dense score`. RRF + boost composes the **sort key**, never the similarity. This is the discipline the floor is calibrated against.
...
- **I-FLOOR-CENTRED.** The three-band gate floor is `CONFIDENCE_FLOOR = 0.75`, half-width `BORDERLINE_HALF_WIDTH = 0.05`. `pass_threshold() = 0.80`; `borderline_threshold() = 0.70` — both returned from the `PASS_CUT` / `FAIL_CUT` constants rather than computed. The canonical window is not itself at risk — `0.75 + 0.05` is exactly `0.8` and `0.75 - 0.05` is exactly `0.7` — but `floor`/`half` are overridable per call and via the `CONFIDENCE_FLOOR` / `BORDERLINE_HALF_WIDTH` env vars, and neighbouring windows are not exact (`0.80 + 0.05` yields `0.8500000000000001`; `0.85 - 0.05` yields `0.7999999999999999`). Defined at `config.py` `CONFIDENCE_FLOOR` / `BORDERLINE_HALF_WIDTH` / `PASS_CUT` / `FAIL_CUT` / `pass_threshold()` / `borderline_threshold()`; consumed in `matching.py` `_gate_thresholds()`, called at the band gate inside `map_vendor_product()` — the public matcher entry point, and the only `map_*` symbol `matching.py` defines. **Calibrated against the Jaro-Winkler distribution.** Opus-prompt scoring is a different distribution; calibration is mandatory before the floor can be trusted under WS-B (see §4).

  > **Corrected 2026-08-16.** This invariant was `I-FLOOR-0.80` and asserted floor `0.80` → `0.85`/`0.75`. That is the pre-5-zone configuration; `config.py:49` has read `0.75` since `docs/superpowers/plans/2026-07-04-scudo-5zone-alignment.md` Task 1. Renamed away from the literal so the ID cannot rot the same way again. Measured, not read:
  > `python3 -c "from scudo_mapping_mcp import config as c; print(c.CONFIDENCE_FLOOR, c.pass_threshold(), c.borderline_threshold())"` → `0.75 0.8 0.7`.
  >
  > Four of the old citation anchors were also wrong, and each pointed at real code, which is how they survived review: `config.py:44` is the IRI-namespace comment, and `matching.py:243-246` is precedent provenance, not the gate. Line-number anchors are not used above — search the symbol names. Note also that `backend/scudo/orchestrator.py:41` declares a **separate** `CONFIDENCE_FLOOR = 0.80` (Runtime-A auto-approve publish gate). Different control, unchanged, do not reconcile.
  >
  > **Corrected 2026-08-17.** The 2026-08-16 fix above swapped rotten line numbers for symbol names and then named a symbol that does not exist: it said the gate is called in `map_product()`. `matching.py` defines no `map_product`; the entry point is `map_vendor_product()`. The name was not invented out of nothing — `backend/routes/mapping.py` really does define a Flask view called `map_product()`, a different layer entirely — which is why it read as plausible. This is the exact failure the rename was meant to stop, recurring one edit later in the same paragraph: a symbol name rots just like a line number when nobody executes the lookup. Measured, not read: `grep -n "^def map_" backend/scudo_mapping_mcp/matching.py` → `159:def map_vendor_product(` (sole hit); `grep -n "_gate_thresholds" backend/scudo_mapping_mcp/matching.py` → defined at `135`, called at `331` (line numbers at the time of writing — search the symbols, not the numbers).
  >
  > **Corrected 2026-08-17 (second).** The same bullet also carried a false *empirical* claim, untouched by either fix above: *"because a naive `0.75 + 0.05` yields `0.8000000000000001` and pushes an exact-0.80 score into BORDERLINE."* It does not. Measured: `python3 -c "print(repr(0.75+0.05), (0.75+0.05)==0.80, repr(0.75-0.05))"` → `0.8 True 0.7` — both canonical edges are exact, so no score was ever at risk at the default window. The rounding in `config.pass_threshold()` is still justified, but for overridden windows: `0.80 + 0.05` → `0.8500000000000001`, `0.85 - 0.05` → `0.7999999999999999`. Origin of the error: when the floor moved `0.80` → `0.75`, the numbers in this sentence were swept forward mechanically without re-measuring (recorded at `.superpowers/sdd/task-1-report.md` RESOLUTION 3). The *old* claim had been true. **The numbers looked like configuration when they were actually measurements** — config numbers travel with a config change, measured ones do not.
...
- **I-SPECIALIST-BORDERLINE-ONLY.** Specialist runs only on `band == borderline`; `confidence = min(dense, specialist)` on concur; disagreement caps `confidence = min(best, borderline_threshold - 0.01)` = **`0.69`** to force `NEEDS_REVIEW`. In `matching.py`: gate at `_gate_thresholds()`, concur-min at the `min(best.similarity, specialist_pick.similarity)` site, and the cap at both `min(best.similarity, borderline_threshold - 0.01)` sites (off-list pick, and specialist **disagreement**). Unchanged in behaviour.

  > **Corrected 2026-08-17 (third).** The bracket above read "(off-list pick, and required-validation failure)". The second cap branch is the specialist **disagreement** path, not a required-validation failure. Read at the two sites: `matching.py:417` is the off-list/invariant-violation branch, and `matching.py:523` sits under the comment *"Specialist DISAGREES — picked a different node from the sparse ranker."* A required-validation failure takes a different branch entirely and does **not** cap `confidence` — it sets `NEEDS_REVIEW` with `confidence = best.similarity` untouched. Found by Codex in the confirmation pass.

  > **Corrected 2026-08-16.** The cap value read `0.74`, derived from the stale `borderline_threshold = 0.75`. At the live `0.70` it is `0.69`. The code computes it — no literal changed — but the doc's number was wrong for anyone reasoning about the gate. The `310, 318, 351` anchors were stale too.

## 4. Calibration plan

The `0.75` floor and ±0.05 half-width (PASS ≥ 0.80 / FAIL < 0.70) were calibrated against the Jaro-Winkler character-similarity distribution. Opus-prompt scoring is a different distribution shape — likely more bimodal (the LLM tends toward confident `0.9+` for clear matches and confident `<0.3` for clear mismatches, with a thinner middle than character-similarity). Without recalibration the band routes the wrong volume to the specialist: too few cases at risk of auto-promoting weak matches, too many at the cost of borderline-specialist invocations.
...
- If the **pass-precision** at `floor=0.75` (pass edge 0.80) is below the I5-target (TBC, expected ≥ 0.95), **lift the floor** to the value where pass-precision crosses the target. Cap the lift at `0.80` (pass edge 0.85); above that, the gate is too restrictive for the demo and a different scoring source is needed.
