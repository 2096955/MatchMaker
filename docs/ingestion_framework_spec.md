...
## Appendix A — MCP-Orchestrated Ingestion Architecture (reference)

Reference diagram for the broader MCP-based ingestion / match-verify / persistence pipeline. The Live Opus agent orchestrates three MCP tools; Persistence is the only writer and is gated by a publish gate that either holds for human review or persists to Neptune for downstream consumption by iFusion.

### Mermaid

```mermaid
flowchart TD
    User[User<br/>read-only results]
    Agent[Live Opus 4.8 agent<br/>orchestrates the MCP tools]

    User <--> Agent

    Agent --> IngestionMCP[Ingestion MCP<br/>normalise any vendor]
    Agent --> MatchMCP[Match & Verify MCP<br/>Falkor check · verifier]
    Agent --> PersistMCP[Persistence MCP<br/>the only writer · gated]

    Vendors[Vendor catalogues<br/>LSEG · Bloomberg · ...]
    Falkor[(FalkorDB<br/>match-and-check graph)]
    Gate{{Publish gate<br/>floor · verifier}}

    IngestionMCP -- any vendor --> Vendors
    MatchMCP -- match + check --> Falkor
    PersistMCP -- every write --> Gate

    Reviewer[Reviewer queue<br/>human in the loop]
    Neptune[(Neptune<br/>canonical RDF store)]
    iFusion[iFusion<br/>consumer · SPI V2]

    Gate -- hold --> Reviewer
    Gate -- persist --> Neptune
    Reviewer -- "approved -> precedent" --> Gate
    Neptune -- SPI V2 --> iFusion
```
...
```
                         +----------------------------+        +-------------------+
                         |   Live Opus 4.8 agent      |<------>|       User        |
                         | orchestrates the MCP tools |        | read-only results |
                         +-------------+--------------+        +-------------------+
                                       |
            +--------------------------+--------------------------+
            |                          |                          |
            v                          v                          v
   +------------------+      +-------------------+      +----------------------+
   |  Ingestion MCP   |      | Match & Verify MCP|      |   Persistence MCP    |
   | normalise vendor |      | Falkor check/verif|      | the only writer/gated|
   +--------+---------+      +---------+---------+      +-----------+----------+
            | any vendor               | match + check              | every write
            v                          v                            v
   +------------------+      +-------------------+      +----------------------+
   | Vendor catalogues|      |     FalkorDB      |      |    Publish gate      |
   | LSEG, Bloomberg  |      | match-and-check   |      |   floor . verifier   |
   +------------------+      +-------------------+      +-----+-----------+----+
                                                              |           |
                                                       hold   |           | persist
                                                              v           v
                                                  +-------------------+  +----------------+
                                                  |  Reviewer queue   |  |    Neptune     |
                                                  | human in the loop |  | canonical RDF  |
                                                  +---------+---------+  +--------+-------+
                                                            |                     |
                                                            | approved ->         | SPI V2
                                                            | precedent           v
...
- **Ingestion MCP** — vendor adapter
- **Match & Verify MCP** — taxonomy / ontology
