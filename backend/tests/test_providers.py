...
def update_provider(provider_id, name, desc=None):
    ...
    if desc:
        payload['provider_desc'] = desc
    r = client.put(f'/api/providers/{provider_id}', json=payload)
    return r.status_code, r.get_json()
...
def test_add_provider_missing_name_returns_400():
    status, data = client.post('/api/providers', json={'provider_desc': 'no name'}), None
...
def test_update_provider_missing_name_returns_400():
    ...
    r = client.put(f'/api/providers/{pid}', json={'provider_desc': 'no name field'})
