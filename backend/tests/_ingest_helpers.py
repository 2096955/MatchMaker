...
def destroy_dataset(ctx):
    ...
    try:
        ...
        with conn.cursor() as c:
            c.execute(
                "DELETE FROM etl_run_log               WHERE dataset_id = %s", (did,)
            )
            c.execute(
                "DELETE FROM tp_dataset_col_transforms  WHERE dataset_id = %s", (did,)
            )
            c.execute(
                "DELETE FROM tp_dataset_col             WHERE dataset_id = %s", (did,)
            )
            c.execute(
                "DELETE FROM tp_dataset                 WHERE dataset_id = %s", (did,)
            )
            c.execute(
                "DELETE FROM tp_provider                WHERE provider_id = %s", (pid,)
            )
        conn.commit()
            ...
            with ing.cursor() as ic:
                ic.execute(
                    sql.SQL("DROP TABLE IF EXISTS {}").format(
                        sql.Identifier(ctx["target_table"])
                    )
                )
            ing.commit()
    finally:
...
def extend_schema(dataset_id, extra_cols):
    ...
    ds_resp = client.get(f"/api/datasets/{dataset_id}", headers=AUTH)
...
def query_ingestion_table(target_table, column=None):
    ...
    ing = get_ingestion_conn()
    try:
        col_expr = sql.Identifier(column) if column else sql.SQL("*")
        query = sql.SQL("SELECT {cols} FROM {table} ORDER BY id").format(
            cols=col_expr, table=sql.Identifier(target_table)
        )
