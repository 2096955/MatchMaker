...
def add_provider(name):
    r = client.post('/api/providers', json={'provider_name': name})
    return r.status_code, r.get_json()


def add_dataset(provider_id, name, landing_folder, fmt='csv',
                pattern='*.csv', columns=None, **kwargs):
    payload = {
...
def update_dataset(dataset_id, provider_id, name, landing_folder,
                   fmt='csv', pattern='*.csv', columns=None, **kwargs):
    payload = {
        'provider_id':       provider_id,
        'dataset_name':      name,
        'data_format':       fmt,
    ...
    payload.update(kwargs)
    r = client.put(f'/api/datasets/{dataset_id}', json=payload)
    return r.status_code, r.get_json()
...
def save_transforms(dataset_id, transforms):
    r = client.put(f'/api/datasets/{dataset_id}/transforms', json=transforms)
    return r.status_code, r.get_json()


def get_transforms(dataset_id):
    r = client.get(f'/api/datasets/{dataset_id}/transforms')
    return r.status_code, r.get_json()
...
def _hard_delete(provider_id, dataset_id=None, target_table=None):
    ...
    try:
        with conn.cursor() as c:
            if dataset_id:
                c.execute("DELETE FROM etl_run_log               WHERE dataset_id = %s", (dataset_id,))
                c.execute("DELETE FROM tp_dataset_col_transforms  WHERE dataset_id = %s", (dataset_id,))
                c.execute("DELETE FROM tp_dataset_col             WHERE dataset_id = %s", (dataset_id,))
                c.execute("DELETE FROM tp_dataset                 WHERE dataset_id = %s", (dataset_id,))
...
def test_add_dataset_missing_required_fields_returns_400():
    status, data = client.post('/api/datasets', json={'dataset_name': 'no provider'}), None
...
def test_add_dataset_same_landing_folder_different_provider_returns_409():
    ...
    try:
        ...
        status2, data2 = add_dataset(prov2['provider_id'], 'DS Prov2', d, pattern='p2_*.csv')
        assert status2 == 409, f"Expected 409, got {status2}"
...
def test_update_dataset_creates_new_version():
    ...
    try:
        ...
        status, data = update_dataset(did, pid, 'Update DS', d,
...
def test_update_dataset_landing_folder_not_exists_returns_400():
    ...
    try:
        ...
        status, data = update_dataset(did, pid, 'Folder Check DS',
                                       '/nonexistent/edit/path')
        assert status == 400, f"Expected 400, got {status}"
...
def test_save_and_get_transforms():
    ...
    try:
        ...
        transforms = [
            {'col_name': 'name',   'trim_option': 'trim',  'null_replace': 'UNKNOWN',
             'case_option': 'upper', 'date_fmt_from': None, 'date_fmt_to': None},
            {'col_name': 'amount', 'trim_option': None,    'null_replace': '0',
             'case_option': None,   'date_fmt_from': None,  'date_fmt_to': None},
            {'col_name': 'id',     'trim_option': None,    'null_replace': None,
             'case_option': None,   'date_fmt_from': None,  'date_fmt_to': None},
        ]
        ...
        shutil.rmtree(d, ignore_errors=True)
    print("PASS: save_transforms / get_transforms — rules saved and retrieved correctly")
...
def test_save_transforms_replaces_previous_rules():
    ...
    try:
        ...
        save_transforms(did, [
            {'col_name': 'name', 'trim_option': 'ltrim', 'null_replace': None,
             'case_option': None, 'date_fmt_from': None, 'date_fmt_to': None},
        ])
        save_transforms(did, [
            {'col_name': 'amount', 'trim_option': None, 'null_replace': '0',
