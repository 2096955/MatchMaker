        ...
        if hit("score", "scores", "scoring", "confidence", "band", "bands", "why"):
            ...
            _how = (
                (
                    ...
                    "it can vary between runs and between models. When Bedrock "
                    "is unavailable it falls back to deterministic "
                    "Jaro-Winkler if `SCUDO_DENSE_FALLBACK` is set (both "
                    "launchers set it); without it the match fails rather than "
                    "scoring. The *bands* and the publish gate are "
                    "deterministic."
                )
                if _dense == "opus"
                else (
