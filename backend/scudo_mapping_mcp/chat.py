...
- You do NOT compute match scores. A deterministic matcher does, using
  Jaro-Winkler similarity plus BM25 retrieval. You call it as a tool and
        ...
        *,
        ...
        if hit("score", "scores", "scoring", "confidence", "band", "bands", "why"):
            ...
            yield say(
                ...
                "numbers are real.\n\n"
                "The score is deterministic, not generated by a language "
                "model. Retrieval nominates candidates with BM25 over the CDAO "
                "catalogue, then each is scored with Jaro-Winkler string "
                "similarity and fused by reciprocal rank fusion. The result is "
                f"banded: PASS at or above {pass_cut:.2f}, BORDERLINE from "
                f"{fail_cut:.2f} to {pass_cut:.2f}, FAIL below {fail_cut:.2f}.\n\n"
                "Because it is deterministic, the same product always scores "
                "the same, and switching the narrating model changes the words "
                "here but never the number."
            )
            yield AgentEvent(type="done", payload={})
            return
