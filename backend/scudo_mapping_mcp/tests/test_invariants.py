...
def test_above_floor_returns_auto_mapped(_isolate_store, clean_frames):
    ...
    ref = VendorProductRef(
        vendor=IN_SCOPE_VENDOR, product_id="EQ-RT-001", name="Equity Prices Real Time"
    )
    result = map_vendor_product(ref)
...
def test_override_records_overridden_status_and_boosts_chosen_node(
    _isolate_store, clean_frames
):
    _isolate_store.set_score(EQ_PRICES_IRI, 0.95)
    ...
    sig = _isolate_store.vendor_signature(
        sibling.vendor, sibling.name, sibling.product_id
    )
    boosts = _isolate_store.rank_signals_for(sig)
    assert boosts.get(EQUITIES_IRI, 0) >= 1, (
        f"rank signal did not record equities override: {boosts}"
    )

    result = map_vendor_product(sibling)
    # EQ_PRICES still wins (much higher base dense score).
    assert result.candidates[0].node.iri == EQ_PRICES_IRI
    # Rank boost tilts SORT only — Candidate.similarity stays the raw dense score (IV).
    eq_node_score = next(
        c.similarity for c in result.candidates if c.node.iri == EQUITIES_IRI
    )
    assert eq_node_score == pytest.approx(0.10)
...
def test_apply_decision_validates_inputs(_isolate_store, clean_frames):
    ...
    with pytest.raises(ValueError, match="decision must be"):
        apply_decision(ref, decision="maybe", decided_by="u", node_iri=EQ_PRICES_IRI)
    with pytest.raises(ValueError, match="decided_by"):
        apply_decision(ref, decision="approve", decided_by="", node_iri=EQ_PRICES_IRI)
    with pytest.raises(ValueError, match="node_iri"):
        apply_decision(ref, decision="approve", decided_by="u", node_iri="")
    with pytest.raises(ValueError, match="suggested_confidence is required"):
        apply_decision(ref, decision="approve", decided_by="u", node_iri=EQ_PRICES_IRI)
...
def test_mapping_artifact_is_self_describing_on_every_path(
    _isolate_store, clean_frames
):
