...
def _frame(vendor, product_id, name='', description=''):
    ...
    if ref is None:
        return VendorProductRef(vendor=vendor, product_id=product_id,
                                name=product_id)
    return ref
...
def get_taxonomy_node(node_iri):
    ...
    try:
        ...
        ui_logger.error('Mapping store unavailable',
...
def find_similar_products():
    ...
    try:
        cands = get_store().find_similar_products(
            ref, max_results=max_results, min_similarity=min_similarity,
        )
...
def record_decision():
    ...
    ui_logger.info(
        ...
        vendor=vendor, product_id=product_id,
        decision=decision_lower, decided_by=decided_by, node_iri=node_iri,
    )
    ...
    err = _validate_vendor(vendor)
    if err:
        ui_logger.warning('HITL mapping decision rejected',
                          vendor=vendor, product_id=product_id,
                          reason=err)
        return jsonify({'error': err}), 400
    ...
    try:
        ...
        ui_logger.warning('HITL mapping decision rejected',
                          vendor=vendor, product_id=product_id,
                          decision=decision_lower, reason=str(e))
...
def import_mapping_bundle():
    ...
    try:
        ...
        # Format incompat, etc.
        ui_logger.warning('Mapping bundle import rejected',
                          principal=g.principal.user_id,
                          source_env=bundle.source_env,
                          reason=str(e))
        return jsonify({'error': str(e)}), 400
    ...
    ui_logger.info(
        ...
        principal=g.principal.user_id,
        source_env=bundle.source_env,
        applied=summary.applied, total=summary.total,
...
def ingest_vendor_file():
    ...
    try:
        ...
        ui_logger.warning('Vendor file ingest rejected',
                          vendor=vendor, filename=f.filename,
                          reason=f'{type(e).__name__}: {e}')
        return jsonify({'error': f'cannot parse vendor file: {e}'}), 400
    except (ConnectionError, TimeoutError, OSError) as e:
        ui_logger.error('Mapping store unavailable',
                        op='ingest', error=f'{type(e).__name__}: {e}')
        return jsonify({'error': 'mapping store unavailable'}), 503

    ui_logger.info(
        'Vendor file ingested',
        principal=g.principal.user_id,
        vendor=vendor, filename=f.filename, products=len(frames),
    )
    return jsonify({
        'ingested': len(frames),
        'products': [
...
def describe_agent():
    ...
    return jsonify({
        'backend': backend,
        'model_id': (
            os.getenv('SCUDO_BEDROCK_MODEL_ID')
            or 'eu.anthropic.claude-opus-4-8'
            if backend == 'bedrock' else None
        ),
        'region': (
...
def run_agent():
    ...
    try:
        ...
        ui_logger.error('Frame data invalid', op='agent_run',
                        vendor=vendor, product_id=product_id,
                        error=f'{type(e).__name__}: {e}')
        return jsonify({'error': f'upstream frame invalid: {e}'}), 502
    except NotImplementedError as e:
    ...
    def event_stream():
        try:
            ...
            ui_logger.error('Mapping store unavailable',
                            op='agent_run', error=f'{type(e).__name__}: {e}')
            yield (
            ...
            ui_logger.error('Agent run failed',
                            op='agent_run', error=f'{type(e).__name__}: {e}')
            yield (
                'data: ' + __import__('json').dumps({
                    'type': 'error',
